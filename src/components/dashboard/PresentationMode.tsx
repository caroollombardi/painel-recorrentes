...
function PresentationSlide({ isActive, children }: { isActive: boolean; children: React.ReactNode }) {
  return (
    <div className={cn(
    ...
    <div className="fixed inset-0 z-50 bg-background flex flex-col">
      <header className="flex items-center justify-between px-6 py-3 bg-card/80 backdrop-blur-sm border-b border-border/50 shrink-0">
        <div className="flex items-center gap-4">
          <img src={wsaLogo} alt="WSA" className="h-7 object-contain" />
        ...
        <div className="flex items-center gap-6">
          ...
          <Button variant="ghost" size="icon" onClick={onExit} className="text-muted-foreground hover:text-foreground h-8 w-8" title="Sair (ESC ou F)">
            <X className="w-4 h-4" />
          </Button>
        </div>
      </header>
        ...
        <PresentationSlide isActive={currentSlide === 0}>
          <div className="h-full overflow-auto p-8 space-y-6">
            <div className="text-center mb-2">
              ...
              <p className="text-sm text-muted-foreground mt-1">Resumo estratégico de clientes recorrentes</p>
            </div>

            {/* Executive Summary in Presentation Mode */}
            <ExecutiveSummary
              data={data}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              <KPICard title="Total Horas Recorrentes" value={`${data.totalHoras.toFixed(1)}h`} subtitle="Contratos fixos mensais" icon={<Clock className="w-5 h-5 text-primary" />} delay={0} />
              <KPICard title="Valor Total Recorrente" value="—" subtitle="Oculto no modo apresentação" icon={<DollarSign className="w-5 h-5 text-primary" />} delay={50} />
              <KPICard
                title="Top Cliente"
                value={data.topClient || "—"}
                subtitle={
                  data.topClient && data.clients.find(c => c.project === data.topClient)?.creditUsage
                    ? generateTopClientPhrase(data.topClient, data.clients.find(c => c.project === data.topClient)?.creditUsage?.percentualUsado || 0, data.topClientHours, data.topClientValor, data.monthProgress)
                ...
                delay={100}
              />
              <KPICard title="Média Valor/Hora" value="—" subtitle="Oculto no modo apresentação" icon={<TrendingUp className="w-5 h-5 text-muted-foreground" />} delay={150} />
              <KPICard
                title="Clientes em Alerta"
                value={`${data.clientsAtWarning + data.clientsAtCritical}`}
                subtitle={
                  data.clientsAtOverflow > 0 ? `🚨 ${data.clientsAtOverflow} estouro, ⚠️ ${data.clientsAtRisk} risco, 🔔 ${data.clientsAtWarning} atenção`
                    : data.clientsAtRisk > 0 ? `⚠️ ${data.clientsAtRisk} risco, 🔔 ${data.clientsAtWarning} atenção`
        ...
        <PresentationSlide isActive={currentSlide === 1}>
          <div className="h-full overflow-auto p-8">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-muted-foreground mt-1">Ordenado do maior para o menor consumo</p>
            </div>
        ...
        <PresentationSlide isActive={currentSlide === 2}>
          <div className="h-full overflow-auto p-8">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-muted-foreground mt-1">Uso de crédito, horas consumidas e valor detalhado</p>
            </div>
      ...
      <div className="h-1 bg-muted shrink-0">
        <div className="h-full bg-primary transition-all duration-700" style={{ width: `${((currentSlide + 1) / slideCount) * 100}%` }} />
      </div>
    </div>
  );
}
